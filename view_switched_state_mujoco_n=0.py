...
'''
python view_switched_state_mujoco_n=0.py --activate muscle1
python view_switched_state_mujoco_n=0.py --activate upper_front
python view_switched_state_mujoco_n=0.py --activate upper_back
python view_switched_state_mujoco_n=0.py --activate lower_front
'''
...
DEFAULT_XML_PATH = os.path.join(os.path.dirname(__file__), "system_model_n=0.xml")
ACT_THRESHOLD = 1e-6
...
def resolve_upper_origin(model: mujoco.MjModel, data: mujoco.MjData, upper_origin_body: Optional[str], upper_origin_xyz: Optional[np.ndarray]) -> np.ndarray:
    ...
    return np.array([-0.025, 0.0, 0], dtype=float)
